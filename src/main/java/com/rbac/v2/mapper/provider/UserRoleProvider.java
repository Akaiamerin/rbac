...
public class UserRoleProvider implements ProviderMethodResolver {
    public String updateUserRoleList(@Param("userRoleList") List<UserRole> userRoleList) {
        StringJoiner insertUserRoleListString = new StringJoiner(", ");
        for (int i = 0; i < userRoleList.size(); i++) {
            StringJoiner insertUserRoleString = new StringJoiner(", ", "(", ")")
            ...
            insertUserRoleListString.add(String.valueOf(insertUserRoleString));
        }
        ...
        StringBuilder sql = new StringBuilder()
                .append("INSERT INTO rbac_user_role ")
                .append("(id, user_id, role_id, status, create_time, update_time) ")
                .append("VALUES ")
                .append(insertUserRoleListString)
                .append(" AS ur ")
    ...
    public String selectUserRoleByUserId(Integer userId) {
        StringBuilder sql = new StringBuilder("SELECT * FROM rbac_user_role WHERE user_id = #{userId}");
        return String.valueOf(sql);
    }
    public String selectUserRoleByRoleId(Integer roleId) {
        StringBuilder sql = new StringBuilder("SELECT * FROM rbac_user_role WHERE role_id = #{roleId}");
        return String.valueOf(sql);
    }
}